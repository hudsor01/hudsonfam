...
function ResetForm() {
  ...
  if (!token) {
    return (
      <div>
        <h1 className="text-xl font-serif text-text mb-2">Invalid Link</h1>
  ...
  return (
    <div>
      <h1 className="text-xl font-serif text-text mb-1">New Password</h1>
      <p className="text-sm text-text-muted mb-6">Enter your new password</p>
      <form onSubmit={handleSubmit} className="space-y-3">
        <input type="password" placeholder="New password" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={8} className="w-full bg-bg border border-border rounded-lg px-3 py-2.5 text-sm text-text placeholder:text-text-dim focus:outline-none focus:border-primary" />
