...
| AI-ACTION-03 | Phase 23 (23-04 CI grep gate + 23-02 triggerCompanyResearch Server Action — requireRole first-line + randomUUID idempotency + sendSignedWebhook("job-company-intel") + discriminated-union return; 23-05 TriggerCompanyResearchButton client component — idle/in-progress/error state machine + setInterval polling + 60-poll cap + unmount cleanup + G-1/G-2/G-3/G-5 grep gates; mount pending 23-06) | Code complete (2026-04-23) — Server Action + client button end-to-end: 9/9 server contract tests (Plan 23-02) + 14/14 client fake-timer tests (Plan 23-05); first fake-timer polling scaffold in project; prod UAT deferred to v3.5 |
| AI-ACTION-04 | Phase 23 (23-04 CI grep gate + 23-02 regenerateCoverLetter Server Action + 23-06 RegenerateCoverLetterButton client component — UPDATE-wait polling with server-returned baseline ISO string, `new Date().getTime()` compare inside predicate body, zero wall-clock reads (G-6 grep = 0), 60-poll cap (D-05), 17/17 fake-timer tests green; UI mount pending 23-08 integration) | Code complete (2026-04-23) — Server Action + client component both landed with full test coverage; D-06 amended baseline template end-to-end across both layers; Phase 24 regenerate buttons can clone this pattern verbatim; prod UAT deferred to v3.5 |
