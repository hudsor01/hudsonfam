...
- [x] **CLOUD-03**: Images served from Cloudflare R2 — uploads (`src/lib/images.ts`) write originals + derivatives to R2; the public read path serves from R2; all NAS/PVC filesystem paths removed; existing photos migrated or the gallery degrades gracefully
- [x] **CLOUD-04**: Homelab-monitoring admin removed entirely (decision 2026-06-01: homelab offline indefinitely, so delete rather than park) — the Glance-replacement dashboard (`src/app/(admin)/admin/page.tsx` + `admin-client`), `/api/dashboard`, the `src/lib/dashboard/*` modules (health, prometheus, server, ups, media, weather, types), the homelab-only admin widgets/tests, and the `/admin` nav link are deleted; the `/admin` route 404s; `SONARR_API_KEY`/`RADARR_API_KEY`/`JELLYFIN_API_KEY` are no longer referenced anywhere. (Preserve the lookalikes: `src/lib/dashboard-actions.ts` content CRUD + `src/components/public/weather-widget.tsx`.) Re-add when the cluster returns — FUTURE-02.
- [x] **CLOUD-05**: Deploy de-homelab'd — `Dockerfile`, `.github/workflows/build-and-push.yml`, `ghcr-retention.yml`, and Flux/K8s references are removed or archived; `bun.lock` matches `package.json` so `bun install --frozen-lockfile` passes
...
- [x] **RECIPE-01**: Recipe search — an instant client-side filter/jump across all recipes by name (cmdk, already installed), reachable from the recipes listing and keyboard-accessible
