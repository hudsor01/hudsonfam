...
- **D-01 [--auto]:** Provision a single GitHub fine-grained PAT with scope `Packages: Read` (read-only), repository scope = `hudsor01/hudsonfam` only (NOT all repos), 1-year expiry, owner = `hudsor01`. Store in vault under key `ghcr-pull-credentials` with two properties: `username: hudsor01` and `pat: <fine-grained PAT value>`. Rationale: works regardless of whether the GHCR package is public or private (private requires it; public ignores it harmlessly); least-privilege scope; matches CICD-05; SEED-005 §Risks already calls out the 1-year expiry tradeoff (calendar reminder 2 weeks pre-expiry — captured as Phase 26 ops note in SUMMARY).
